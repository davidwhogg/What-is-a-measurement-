% Copyright 2025, 2026 David W. Hogg. All rights reserved.

% to-do items
% -----------
% - figure out whether figures will be marginal, and either way, put in some figures and captions.
...
% typesetting and page layout issues
\usepackage[letterpaper, text={5.00in,9.00in}]{geometry}
\usepackage{setspace}
...
% draft and communication macros -- TO BE DELETED
\usepackage{xcolor}
...
This Mission is trying to deliver valuable information to the astronomical community, members of which can make measurements of, or update their beliefs about, individual stars, stellar clusters, and populations of stars, all using the \textsl{Gaia} data releases.
...
In contexts like this, there are many meanings of the word ``model,'' but for me the model will be something that can be used to compute probabilities in the space of the data.
...
How Bayesian priors and posteriors act under reparameterizations will come back up in \secref{sec:bayes} and \secref{sec:flat}.
...
Notice that this is all a generalization of the discussion of linear fitting above in \secref{sec:lf} around and after equation~\eqref{eq:Ca}.
...
The second is that we live in the future, in which programming languages like jax \cite{jax} and julia \cite{julia} give you analytic derivatives to second order for free.
...
That's demonstrated in \hogg{what FIGURE?} and definitely not what people usually have in mind when they think about using ``uninformative priors.''
...
In many cases, the likelihood function will have not one peak, but two or many.
One common example is in time-series analysis:
If you are looking for a frequency in a time series, you will generally have many tiny peaks in the likelihood function at a set of different frequencies.
If one peak is much taller than all the other peaks---for example if the difference in (natural) log likelihood between the tallest peak and the second-tallest peak is much greater than unity---then you have a good measurement, and you can do everything you would usually do:
You can choose the maximum-likelihood peak as your measurement, and the formal uncertainty region can be established with log-likelihood differences (or second derivatives of the log-likelihood function).

When there are multiple peaks, and the peaks differ in height (in the log-likelihood sense) by of order unity (like say delta-log-likelihood of 0.75 or 1.3), the situation is more challenging.
\hogg{What to say here?}
...
This isn't true, and besides, once the feature space gets large enough (the data lives in a high-dimensional space), coverage of that kind isn't possible to satisfy.
...
\section{Assumptions, choices, and community norms}\label{sec:assumptions}
If your data $Y$ are going to be measuring your parameter of interest $\theta$, then it really must be that there is a \emph{causal connection} between $Y$ and $\theta$.
Statisticians sometimes get bent out of shape when the ``causal'' word arises here; I am not talking about causal inference, I am talking about causal structure:
It has to be that the expectation (or some moment) of $Y$ changes as $\theta$ changes, otherwise there will be no possible measurement.
After all, measurement is related to the likelihood principle, and the likelihood is the probability for the data, conditioned on the parameters.
If there is no dependence, there is no measurement.

One (perhaps unimportant) comment to make here is that causal relationships are generally counterfactual, in the sense that they are related to the question:
``If $\theta$ had a different value, how would my expectations about $Y$ change?''
It is almost always the case that if some parameter is entering your likelihood function, then there are counterfactual questions you can ask and answer.

Another comment to make here is that we almost never know, with high confidence, the causal structure of the world.
This point is related to causal inference:
It is hard to know, with certainty, that anything causes anything else.
Or, more correctly, we aren't usually spending lots of our time in science in areas where the causation is fully settled.\footnote{My colleague Jennifer Hill (NYU) says ``It is hard to think of any question in science that isn't fundamentally causal.''}
So is it possible for us to \emph{know} that $\theta$ is causally related to $Y$, such that a measurement will even be possible?
How could we know this, if causation is often uncertain?

The answer is (and foreshadowed in \secref{sec:intro}) that all measurements require \emph{assumptions}.
Every measurement requires a model for how the world works, and preferably a model that is specific enough that it specifies a likelihood function $p(Y\given\theta,\alpha,I)$.
How do we make these assumptions?
If we want our measurement to be useful to scientists and accepted by scientists, we must make these assumptions at least largely in accordance with the norms and expectations of our field.
And, consequently, every measurement we ever make is \emph{conditional on myriad asumptions}.

The amazing thing is that every measurement humanity has ever made is conditioned on myriad assumptions, and yet we have an incredibly successful model of the world, and our measurements have survived lots of changes to our basic theory.
When we drop something, we can predict very accurately how it will fall, and when we launch a satellite or space probe, we can predict very accurately what its trajectory will be.
It is hard to know whether our measurements are correct---and they can only be correct subject to assumptions---but we make very good predictions using our measurements and the theories that underly them.
Further discussion of that point is out of scope.

How do we describe or track or communicate the assumptions---often primarily causal assumptions---that go into our measurements?
\hogg{Rix notes that if there isn't a arrow (or chain of arrows) in a causal graph that links parameters of interest $\theta$ to data $y$, then, \emph{even if you can write down a likelihood function}, you might not have a valid measurement.
Think about this and write something here about this question.}

Finally, I want to make a comment about ``community norms.''
A lot of progress has been made in science---and in human affairs generally---by people who are willing to do away with community norms and think and act radically.
I fully support this, and nothing in this \documentname{} should be seen as arguing against that.
In my own work, I have worked on technosignatures \cite{techno} and tests for inhomogeneous cosmologies \cite{fractal}.
But these projects aren't projects that deliver \emph{measurements}.
Measurements can only really be made within the context of an established or sensible model.
We must obviously always endeavor to critically test and attack the models we use; this is among the most important scientific work that anyone ever does.
Measurement is just a tiny bit of science, and it only happens when models are quantitative and mature.
Thus the scope of this whole \documentname{} is very limited.
...
An example from astrophysics might be that each data set $Y_i$ is the data on an exoplanet $i$, measuring the parameters $\theta_i$ of that planet; the parameters $\beta$ describe the distribution or population of planets.
...
In detail, when any integral or product of integrals is implemented, it is important (for numerical stability) to keep all pdfs in logarithmic form, so really the integrals in \eqref{eq:hierarchical} ought to be performed with something like a \texttt{logsumexp}, and combined with a sum.\footnote{\hogg{Put more here?}}
...
\section{Delivering measurements useful to others}\label{sec:catalogs}
...
So while this Bayesian point of view (release posterior pdfs and also fully executable code) is self-consistent and rigorously correct, it isn't compatible with the zeitgeist in many academic disciplines at the moment.\footnote{\hogg{Something about making the community better here.}}
...
This discussion should refer back to things in \secref{sec:flat}, where \hogg{I hope that} we said things about how to safely look at the ratio of the prior to the posterior in high dimensions?
...
A measurement, for the purposes of a scientific paper, is (in some sense) a value (the result of the measurement), and an uncertainty, that can be reported in a paper abstract.
What should these two reported numbers (value and uncertainty) be?
In an ideal world, the reported value should be the maximum $\hat\theta$ of the likelihood \eqref{eq:ML}, and the uncertainty should be related to the Fisher information or else level sets of the log-likelihood function.
As we described in \secref{sec:inference}, the formal uncertainty on the parameter $\theta$ is the square root of the inverse of the negative of the second derivative tensor, evaluated at that maximum.
...
The first is that if anything strange was used for prior pdfs, especially prior pdfs on nuisance parameters, then that weird context must additionally be reported in the abstract.
...
Often, when a measurement is made, we want to know ``how many sigma'' it is away from a standard value, or a previous measurement.
...
(This is another reason that it might be a bad idea to adopt a ``number of sigma'' threshold to claim a measurement.)
But once again, if you want them, upper limits can be constructed with log-likelihood differences, as described in \secref{sec:lf}.
...
\hogg{But above all, I am not restricting statistical practice: Do what you like! Just remember that a confident claim of data $Y$ measuring parameter $\theta$ requires a peak in a properly profiled likelihood function, or some reliable approximation thereto.}

\hogg{Maybe end on the point that the document discusses the beliefs of the community.
Bayes is the language and calculus of beliefs.
Thus maybe we should be thinking about the community as being the entity that has beliefs?}
