...
% to-do items
% -----------
% - Claude says that the likelihood principle is Bayesian and NOT frequentist. Address this point!
% - write all sections.
...
The problem setup will be that there is a list or block of data $Y$, an ordered list of $p$ parameters of interest $\theta$, an ordered list of $s$ nuisance parameters $\alpha$, and a large set of choices, assumptions, and background information $I$.
...
such that the expectation for the data $Y$ can be written as $X\,\theta$ for some $n\times p$ (HOGG WE NEED TO DEFINE $p$ and $s$ maybe as sizes for $\theta$ and $\alpha$) rectangular design matrix $X$,
and when the noise is Gaussian with zero mean and known covariance tensor $C$.
...
Then you take the (square) $p\times p$ submatrix of $C_{(\theta,\alpha)}$ corresponding to the $p$ parameters in $\theta$ only.\footnote{It is critical here that you invert before you take the submatrix. If you submatrix before you invert, you will get disastrously wrong uncertainty estimates. In that (wrong) case you have computed the best possible uncertainty under the (wrong) assumption that God told you the correct values of the nuisance parameters.}
...
If $p>1$---that is, if $\theta$ has more than one component---this $p\times p$ submatrix is the variance tensor of the uncertainty ellipse in the $\theta$ space.
...
That is, the raw-data likelihood can be very simply related to the derived-data likelihood, provided that the investigator has access to the intermediate likelihood function $p(Z\given Y,I)$ (or enough information to reconstruct it).
...
In this case, the model is… HOGG HAVE WE DONE THIS ALREADY ABOVE? IF NOT, THIS NEEDS TO GO IN ABOVE.
